...
exp_name = "randomnklandscapes_2"

# An hour should be sufficient, if it takes longer...
time_limit = 60 * 60 # s
...
# Greedy Scatter Subset -> Nearest Clustering
...
approaches = t_f_mfs
len_approaches = len(t_f_mfs)
print(f"{len_approaches} approaches")

print(f"Total: {len_functions * len_approaches} runs.")
...
def generate_experiment(spec):
    ...
    (topology, distance, fostype, multifos) = approach
    ...
    return (
        "python3 "
        "run_gom_population_sequential.py "
        f"--base-folder={folder} "
        f"--num=10 "
        f"--base=16 "
        f"--max={max_population} "
