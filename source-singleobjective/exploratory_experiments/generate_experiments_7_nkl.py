...
idx_extractor = re.compile(r"([0-9]+).txt$")
L_extractor = re.compile(r"^L([0-9]+)$")
n_and_s_extractor = re.compile(r"^n([0-9]+)_s([0-9]+)$")
...
def extract_info(p: Path):
    ...
    L = L_extractor.match(p.parts[-2]).group(1)
    n_and_s_match = n_and_s_extractor.match(p.parts[-3])
    n = n_and_s_match.group(1)
    s = n_and_s_match.group(2)

    # Get vtr file and parse it (and subtract epsilon)
    p_vtr = p.parent / f"{idx}_vtr.txt"
    vtr = 0.0
    ...
    return (p, idx, L, n, s, vtr)

# n = k?
# List of (instance path, index, string length, n, s)
instances = list(map(extract_info, filter(is_not_vtr_file, Path("./data/nk/instances").glob("*/*/*.txt"))))
functions = instances
...
def generate_experiment(spec):
    ...
    folder = (
        f"results/"
        f"{exp_name}/"
        f"L{L}__n{n}__s{s}__idx{idx}__"
