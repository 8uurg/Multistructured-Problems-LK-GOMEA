...
exp_name = "investigate-localsearch"

approaches = [
    # No LS, default configuration
    ...
    # SIHC - Random Direction
    "0 -c0 -l1_0", # Original MO-GOMEA
    "0_-1 -c8 -x-1 -l1_0", # + KNN
    "0_-1 -c9 -x-1 -l1_0", # + Symmetric KNN (i.e. given solutions `a` and `b`, a is KNN of b, or b is KNN of a)
    "-3_-1 -c0 -l1_0", # Scalarized MO-GOMEA
    "-3_-1 -c8 -x-1 -l1_0", # + KNN
    "-3_-1 -c9 -x-1 -l1_0", # + Symmetric KNN
    # EHC - Random Direction
    "0 -c0 -l1_1", # Original MO-GOMEA
    "0_-1 -c8 -x-1 -l1_1", # + KNN
    "0_-1 -c9 -x-1 -l1_1", # + Symmetric KNN (i.e. given solutions `a` and `b`, a is KNN of b, or b is KNN of a)
    "-3_-1 -c0 -l1_1", # Scalarized MO-GOMEA
    "-3_-1 -c8 -x-1 -l1_1", # + KNN
    "-3_-1 -c9 -x-1 -l1_1", # + Symmetric KNN
]
...
ls = [12, 25, 50, 100]
#
max_number_of_evaluations = 5_000_000
writing_interval = 100_000
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, (l, (fns, bot_instance_0, bot_instance_1)) in product(approaches, range(run_start_index, run_start_index + number_of_runs), ns_and_instances_bot_bot_6):
            ...
            f.write((
                ...
                f"-f{approximation_front_file} " # front approximation
                f"-R{random_seed} " # random seed to use
                "7 " # problem index
                "2 " # number of objectives
                f"{l} " # number of parameters
                "1000 " # elitist archive size target
