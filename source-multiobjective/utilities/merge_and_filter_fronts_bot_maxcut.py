...
# TODO: EDIT: Construct instances.
# Ensure int
files["fn"] = files["fn"].astype(int)
# Construct mapping for fns <-> fn
instance_combinations = pd.DataFrame(chain.from_iterable(
    ({"fns": fns, "fn": fn} for fn in range(fns))
    for fns in [1, 2, 4, 8, 16]
))
files = pd.merge(files, instance_combinations, on="fn")


# Columns to merge.
columns_to_merge = ["file", "approach", "run", "fn"]
...
def perform_merger(df: pd.DataFrame):
    ...
    filename = f"{stem}__s_0__k_5.txt.txt"
    files_in = list(df["file"])
    file_out = directory_out / filename
     # EDIT: Provide instance
    n = attributes["n"]
    fns = attributes["fns"]
    maxcut_instance_0 = f"./maxcut/maxcut_instance_{n}_0.txt"
    bot_instance_0 = f"./bestoftraps/bot_n{n}k5s0fns{fns}.txt"
    instance = f"b_{bot_instance_0};c_{maxcut_instance_0}"
