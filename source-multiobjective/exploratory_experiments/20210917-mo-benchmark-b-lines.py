...
# This experiment is an extension of `20210906-mo-benchmark-b.py`, which evaluates variants of the two reference approaches
# replacing the clustering with a line-based approach.
# Furthermore, this also performs the HV-based experiments.
#
...
# `-3` - Tschebysheff Scalarized MO-GOMEA
...
#
#  `-2` - Domination-based MO-GOMEA - like 
...
#
#  `-4`, `-5`, `-6`, `-7` - HV MO-GOMEA - GOMEA, but with acceptation based on the method described in:
...
#       
#       `-6` and `-7` compute HV in a steady state like manner, more costly, but up-to-date.
#       `-4` and `-6` perform Euclidean Clustering, while `-5` and `-7` perform line clustering.
#
#   Approaches with -c6 added use Hamming Clustering.
approaches = ["-2", "-3", "-4", "-5", "-6", "-7", "-8", "-9", "-10", "0 -c6", "-3 -c6", "-4 -c6", "-6 -c6", "-8 -c6", "-10 -c6"]
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, n in product(approaches, range(number_of_runs), ns):
            ...
            f.write((
                "./build/MO_GOMEA "
                f"-a{a} " # approach
                f"\"-oresults/mo_benchmark_b_onemax_vs_zeromax/approach_{a}__run_{run_idx}__n_{n}\" " # output folder
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, (n, maxcut_instance_0) in product(approaches, range(number_of_runs), ns_and_instances_maxcut_2):
            ...
            f.write((
                "./build/MO_GOMEA "
                f"-a{a} " # approach
                f"-oresults/mo_benchmark_b_maxcut_vs_onemax/approach_{a}__run_{run_idx}__n_{n} " # output folder
                f"-s{population_size}_{num_clust} " # population size & number of clusters
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, n in product(approaches, range(number_of_runs), ns):
            ...
            f.write((
                "./build/MO_GOMEA "
                f"-a{a} " # approach
                f"-oresults/mo_benchmark_b_maxcut_vs_maxcut/approach_{a}__run_{run_idx}__n_{n} " # output folder
                f"-s{population_size}_{num_clust} " # population size & number of clusters
                "4 " # problem index
                "2 " # number of objectives
                f"{n} " # number of parameters
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, (n, fns, bot_instance_0) in product(approaches, range(number_of_runs), ns_and_instances_bot_4):
            ...
            f.write((
                "./build/MO_GOMEA "
                f"-a{a} " # approach
                f"-oresults/mo_benchmark_b_bot_vs_onemax/approach_{a}__run_{run_idx}__n_{n}__fns_{fns} " # output folder
                f"-s{population_size}_{num_clust} " # population size & number of clusters
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, (n, fns, bot_instance_0, maxcut_instance_0) in product(approaches, range(number_of_runs), ns_and_instances_bot_maxcut_5):
            ...
            f.write((
                "./build/MO_GOMEA "
                f"-a{a} " # approach
                f"-oresults/mo_benchmark_b_bot_vs_maxcut/approach_{a}__run_{run_idx}__n_{n}__fns_{fns} " # output folder
                f"-s{population_size}_{num_clust} " # population size & number of clusters
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, (n, (fns, bot_instance_0, bot_instance_1)) in product(approaches, range(number_of_runs), ns_and_instances_bot_bot_6):
            ...
            f.write((
                "./build/MO_GOMEA "
                f"-a{a} " # approach
                f"-oresults/mo_benchmark_b_bot_vs_bot/approach_{a}__run_{run_idx}__n_{n}__fns_{fns} " # output folder
                f"-s{population_size}_{num_clust} " # population size & number of clusters
