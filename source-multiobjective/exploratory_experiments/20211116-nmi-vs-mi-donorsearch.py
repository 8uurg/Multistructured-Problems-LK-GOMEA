...
approaches = [
    "-3_-1 -c0 -d", # Scalarized MO-GOMEA
    "-3_-1 -c8 -x-1 -d", # + KNN
    "-3_-1 -c9 -x-1 -d", # + Symmetric KNN
    "-3_-1 -c0 -N -d", # Scalarized MO-GOMEA + NMI (instead of MI)
    "-3_-1 -c8 -x-1 -N -d", # + KNN + NMI (instead of MI)
    "-3_-1 -c9 -x-1 -N -d", # + Symmetric KNN + NMI (instead of MI)
]
