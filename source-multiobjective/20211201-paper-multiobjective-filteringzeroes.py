...
approaches = [
    # Original MO-GOMEA
    "0_-1 -c10 -x-1", # + KNN
    "0_-1 -c11 -x-1", # + Symmetric KNN (i.e. given solutions `a` and `b`, a is KNN of b, or b is KNN of a)
    # Scalarized MO-GOMEA
    "-3_-1 -c10 -x-1", # + KNN
    "-3_-1 -c11 -x-1", # + Symmetric KNN
]
...
run_maxcut_vs_maxcut  = False # Disable: Only need one comparison for now
run_bot_vs_onemax     = False # Disable: No Onemax (keep down number of problems)
run_bot_vs_maxcut     = False # Disable: Only need one comparison for now
run_bot_vs_bot        = True
    ...
    with open('experiments.txt', file_mode) as f:
        for a, run_idx, l in product(approaches, range(run_start_index, run_start_index + number_of_runs), ls):
            ...
            f.write((
                ...
                f"-R{run_idx + 42} " # random seed provided for determinism
                "0 " # problem index
